...
function openEditItem(item) {
  ...
  itemForm.value = {
    ...
    status: item.status,
    purchase_options: purchaseOptionsFromItem(item),
    notes: item.notes || '',
  }
...
<template>
  ...
  <div v-else class="max-w-5xl mx-auto p-4 pb-20 space-y-6">
    ...
    <div
      v-if="actionSuccess"
      class="bg-green-950/50 border border-green-900 text-green-200 rounded-xl px-4 py-3 flex items-start justify-between gap-3"
    >
      <p class="text-sm">{{ actionSuccess }}</p>
      <button @click="actionSuccess = ''" class="text-xs text-green-300 hover:text-green-100 transition-colors">
        Dismiss
      </button>
    </div>
    ...
    <div
      v-if="actionError"
      class="bg-red-950/60 border border-red-900 text-red-200 rounded-xl px-4 py-3 flex items-start justify-between gap-3"
    >
      <p class="text-sm">{{ actionError }}</p>
      <button @click="actionError = ''" class="text-xs text-red-300 hover:text-red-100 transition-colors">
        Dismiss
      </button>
    </div>
    ...
    <div class="flex gap-1 bg-gray-900 rounded-xl p-1 border border-gray-800">
      <button
        v-for="tab in ['rooms', 'items', 'budget']"
        :key="tab"
        @click="activeTab = tab"
    ...
    <div
      ...
      @click.self="!savingRoom && (showRoomForm = false)"
    >
      <form
